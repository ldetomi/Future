$( document ).ready(function() {
	...
	$("#iniziative-e-novita").owlCarousel({
		...
		responsive:{
		  ...
		  600:{  //Questo ï¿½ un breakpoint aggiuntivo per questo carosello, non ï¿½ comune al resto del sito
		      items:2
		  },
		  768:{
		      items:2
    ...
    //Eseguiamo la prima volta al Ready() in caso in cui il sito venga aperto direttamente in modalitï¿½ Mobile
    ...
    //Eseguiamo la prima volta al Ready()
    make_YoutubeIframes_responsive();

    //Eseguiamo anche al Resize della finestra
    $(window).resize( make_YoutubeIframes_responsive );
    ...
    //Lo script seguente IMITA gli Accordion jQuery UI aggiungendo dinamicamente le classi apposite, ma consente anche l'apertura simultanea di piï¿½ sezioni
    ...
    $(".sisal-accordion").addClass("ui-accordion ui-widget ui-helper-reset").find("> h2")
        ...
        .prepend('<span class="ui-accordion-header-icon ui-icon ui-icon-carat-1-s"></span>').click(function() {
             ...
             //Eseguiamo la funzione che valuta la responsivitï¿½ delle immagini floattanti ogni volta che viene aperto un Accordion
             addClearingToImages_inResponsiveMode;
	...
	$('.fakeEmailReset').keyup(function() {
		$('.fakeEmailReset').val($(this).val());
		$('#_58_realEmailReset').val($('.fakeEmailReset').val());
	});
	...
	//Rimuove il Loading dalla Form di Default di ForgotPassword che ï¿½ nascosta
...
//Rende le immagini "non float" in modalità Responsive
...
//Rende Responsive gli iframes contenenti i video di Youtube
function make_YoutubeIframes_responsive()
{
  $('iframe[src*="youtube"]').each(function( index ) {

    var $video_original_width = $(this).attr('width');
    var $video_original_height = $(this).attr('height');

    var $video_ratio = $video_original_width / $video_original_height;

    var $video_actual_width = $(this).width();
    var $video_calculated_height = $video_actual_width / $video_ratio;

    $(this).css('height',$video_calculated_height);
  });
}
...
// Questa funzione puï¿½ essere richiamata nelle chiamate Ajax per mostrare uno sfondo bianco a tutto schermo durante l'esecuzione.
